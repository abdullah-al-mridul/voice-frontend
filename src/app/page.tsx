...
export default function HomePage() {
  ...
  const handleMockAuth = (e: React.FormEvent) => {
    e.preventDefault();
    // Save dummy state to localStorage
      ...
      <div className="w-full max-w-[1250px] h-full flex flex-col lg:flex-row bg-transparent overflow-hidden mx-auto">
        ...
        <div className="w-full lg:w-[40%] xl:w-[35%] h-full bg-transparent relative z-10 overflow-hidden flex flex-col">
          <div className="flex-grow relative w-full h-full">
              ...
              <div className="max-w-md mx-auto w-full">
                ...
                <form className="space-y-4" onSubmit={handleMockAuth}>
                  <div>
                    ...
                    <input className="w-full px-4 py-3 rounded-none bg-white dark:bg-surface-dark border border-gray-200 dark:border-border-dark focus:border-primary focus:ring-primary dark:text-white shadow-sm transition-colors" id="full-name" name="full-name" placeholder="John Doe" type="text" required />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1" htmlFor="email">Email Address</label>
                    <input className="w-full px-4 py-3 rounded-none bg-white dark:bg-surface-dark border border-gray-200 dark:border-border-dark focus:border-primary focus:ring-primary dark:text-white shadow-sm transition-colors" id="email" name="email" placeholder="john@example.com" type="email" required />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1" htmlFor="username">Username</label>
                      <input className="w-full px-4 py-3 rounded-none bg-white dark:bg-surface-dark border border-gray-200 dark:border-border-dark focus:border-primary focus:ring-primary dark:text-white shadow-sm transition-colors" id="username" name="username" placeholder="@johnny" type="text" required />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1" htmlFor="dob">Date of Birth</label>
                      <input className="w-full px-4 py-3 rounded-none bg-white dark:bg-surface-dark border border-gray-200 dark:border-border-dark focus:border-primary focus:ring-primary dark:text-white shadow-sm transition-colors" id="dob" name="dob" type="date" required />
                    </div>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input className="w-full px-4 py-3 rounded-none bg-white dark:bg-surface-dark border border-gray-200 dark:border-border-dark focus:border-primary focus:ring-primary dark:text-white shadow-sm transition-colors" id="password" name="password" type="password" required />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1" htmlFor="confirm-password">Confirm</label>
                      <input className="w-full px-4 py-3 rounded-none bg-white dark:bg-surface-dark border border-gray-200 dark:border-border-dark focus:border-primary focus:ring-primary dark:text-white shadow-sm transition-colors" id="confirm-password" name="confirm-password" type="password" required />
                    </div>
                  </div>
                  <div className="flex items-start mt-2">
                    <div className="flex items-center h-5">
                      <input className="h-5 w-5 text-primary border-gray-300 dark:border-border-dark rounded-none focus:ring-primary bg-white dark:bg-surface-dark" id="terms" name="terms" type="checkbox" required />
                    </div>
                    <div className="ml-3 text-sm">
                      <label className="font-medium text-gray-700 dark:text-gray-300" htmlFor="terms">I agree to the <a className="text-primary hover:underline" href="#">Terms of Service</a> and <a className="text-primary hover:underline" href="#">Privacy Policy</a>.</label>
                    </div>
                  </div>
            ...
            {/* OTP Flow Omitted for brevity, but same structure as before... */}
          </div>
        </div>

        {/* Right Section - Auto Scrolling Posts */}
          ...
          <div className="h-full w-full grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6 px-4">
            <div className="relative overflow-hidden h-full">
              <div className="animate-scroll-up flex flex-col gap-6">
                {[...leftColumnPosts, ...leftColumnPosts].map((post, idx) => (
            ...
            <div className="relative overflow-hidden h-full">
              <div className="animate-scroll-down flex flex-col gap-6">
                {[...rightColumnPosts, ...rightColumnPosts].map((post, idx) => (
