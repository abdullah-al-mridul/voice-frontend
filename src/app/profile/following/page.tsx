        ...
        <main className="flex-1 max-w-2xl w-full border-r border-border dark:border-[#2F3336] min-h-screen overflow-y-auto no-scrollbar">
          <div className="sticky top-0 bg-background/80 dark:bg-[#0F0F0F]/80 backdrop-blur-md z-40 border-b border-border dark:border-[#2F3336]">
            ...
            <div className="flex w-full mt-2">
              ...
              >
                Followers
                {activeTab === 'followers' && <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-14 h-1 bg-primary rounded-full"></div>}
              </button>
              <button 
          ...
          <div className="pb-20">
            {followSuggestions.map((user) => (
              <div key={user.id} className="p-4 border-b border-border hover:bg-black/5 dark:hover:bg-white/5 cursor-pointer transition-colors flex gap-3">
                <img alt="Avatar" className="w-12 h-12 rounded-full object-cover flex-shrink-0" src={user.avatar} />
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start">
                    <div className="flex flex-col min-w-0">
                      <div className="flex items-center gap-1">
                        <span className="font-bold hover:underline truncate">{user.name}</span>
                        {user.verified && <i className="fas fa-check-circle text-primary text-[10px]"></i>}
                      </div>
                  ...
                  <p className="text-sm mt-1 leading-normal line-clamp-2">
                    Digital explorer, tech enthusiast, and professional VOICEr. Building the future of social connection. 🚀
                  </p>
                </div>
              </div>
            ))}
