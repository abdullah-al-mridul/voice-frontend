        ...
        <main className="flex-1 max-w-2xl w-full border-r border-border dark:border-[#2F3336] min-h-screen overflow-y-auto no-scrollbar">
          <div className="sticky top-0 sm:top-0 bg-background/80 dark:bg-[#0F0F0F]/80 backdrop-blur-md z-40 border-b border-border dark:border-[#2F3336]">
            ...
            <div className="flex w-full overflow-x-auto no-scrollbar snap-x snap-mandatory">
              <ExploreTab label="For you" active={activeTab === 'for-you'} onClick={() => setActiveTab('for-you')} />
              <ExploreTab label="Trending" active={activeTab === 'trending'} onClick={() => setActiveTab('trending')} />
              <ExploreTab label="News" active={activeTab === 'news'} onClick={() => setActiveTab('news')} />
              <ExploreTab label="Sports" active={activeTab === 'sports'} onClick={() => setActiveTab('sports')} />
              <ExploreTab label="Entertainment" active={activeTab === 'entertainment'} onClick={() => setActiveTab('entertainment')} />
            </div>
          </div>
          ...
          <div className="pb-20">
            ...
            <div className="relative aspect-[16/9] w-full overflow-hidden cursor-pointer group">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent flex flex-col justify-end p-4">
                <span className="text-white/80 text-sm font-medium mb-1">Politics · Live</span>
            ...
            <div className="border-b border-border dark:border-[#2F3336] py-3">
              ...
              {trendingTopics.map(topic => (
                <div key={topic.id} className="hover:bg-black/5 dark:hover:bg-white/5 px-4 py-3 cursor-pointer transition-colors relative group">
                  <div className="flex justify-between text-xs text-muted-foreground">
                    <span>{topic.category}</span>
                    <MoreHorizontal className="w-4 h-4 opacity-0 group-hover:opacity-100 transition-opacity" />
                  </div>
                  <div className="font-bold text-base mt-0.5">{topic.topic}</div>
                ...
                <div key={item} className="flex gap-4 cursor-pointer group">
                  <div className="flex-1">
                    ...
                    <h4 className="font-bold text-sm line-clamp-2 group-hover:underline">
                      New AI model breakthroughs show promise in early medical diagnosis and treatment planning.
                    </h4>
                  </div>
                  <div className="w-16 h-16 rounded-xl overflow-hidden bg-muted flex-shrink-0">
                    <img src={`https://picsum.photos/seed/tech_${item}/100/100`} alt="News" className="w-full h-full object-cover" />
                  </div>
                </div>
              ))}
            </div>
...
function ExploreTab({ label, active, onClick }: { label: string, active: boolean, onClick: () => void }) {
  return (
    <button 
      onClick={onClick}
      className={cn(
    ...
    >
      {label}
      {active && <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-12 h-1 bg-primary rounded-full"></div>}
    </button>
  );
}
