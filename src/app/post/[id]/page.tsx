...
export default function PostDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [currentUser, setCurrentUser] = useState<{ name: string; handle: string; avatar: string } | null>(null);
  ...
  if (!post) return (
    <div className="h-screen w-full flex flex-col items-center justify-center bg-background text-foreground">
      ...
      <Link href="/feed" className="bg-primary text-white px-6 py-2 rounded-full font-bold">
        Back to Home
      </Link>
    </div>
  );
            ...
            <div className="flex flex-col h-full p-4">
              <div className="mb-6 px-2 flex flex-col gap-3">
                <div className="flex items-center gap-3">
                  <img alt="User" className="w-10 h-10 rounded-full object-cover" src={currentUser?.avatar} />
                  <div className="flex flex-col">
                    <span className="font-bold text-sm">{currentUser?.name}</span>
                    <span className="text-muted-foreground text-xs">{currentUser?.handle}</span>
                  </div>
                </div>
          ...
          <article className="p-4 space-y-4">
            ...
            {post.image && (
              <div className="rounded-2xl overflow-hidden border border-border dark:border-[#2F3336] mt-4">
                <img alt="Post content" className="w-full h-auto object-cover max-h-[700px]" src={post.image} />
              </div>
            )}
            ...
            <div className="py-4 border-b border-border dark:border-[#2F3336] flex gap-4 text-sm">
              <div className="flex gap-1">
                <span className="font-bold text-foreground">{post.stats.reposts}</span>
              ...
              <div className="flex gap-1">
                <span className="font-bold text-foreground">{post.stats.likes}</span>
            ...
            <div className="flex justify-around items-center py-2 border-b border-border dark:border-[#2F3336] text-muted-foreground">
              <ActionItem icon={MessageCircle} hoverColor="hover:text-primary" />
              <ActionItem icon={Repeat} hoverColor="hover:text-green-500" />
              <ActionItem icon={Heart} hoverColor="hover:text-pink-600" />
              <ActionItem icon={Bookmark} hoverColor="hover:text-primary" />
              <ActionItem icon={Share} hoverColor="hover:text-primary" />
            </div>
                ...
                <div className="flex justify-between items-center mt-4">
                  ...
                  <button 
                    ...
                    className={cn(
                      ...
                      replyText.trim() ? "bg-primary text-white" : "bg-primary/50 text-white cursor-not-allowed"
                    )}
                  >
                    Reply
                  </button>
        ...
        <aside className="w-[350px] hidden lg:block h-screen sticky top-0 px-6 py-4 space-y-4 overflow-y-auto no-scrollbar">
          ...
          <div className="bg-secondary dark:bg-[#16181C] rounded-2xl border border-border dark:border-[#2F3336] pt-3">
            ...
            <div className="flex items-center justify-between px-4 py-3 hover:bg-black/5 dark:hover:bg-white/5 cursor-pointer transition-colors">
              ...
              <button className="bg-foreground text-background text-sm font-bold px-4 py-1.5 rounded-full transition-colors">Follow</button>
            </div>
          </div>
        </aside>
      </div>
...
function ActionItem({ icon: Icon, hoverColor }: { icon: any, hoverColor: string }) {
  return (
    <button className={`p-2 rounded-full transition-colors ${hoverColor} hover:bg-current/10`}>
